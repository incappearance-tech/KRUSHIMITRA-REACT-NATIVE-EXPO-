...
export const useTransporterStore = create<TransporterStore>((set) => ({
    ...
    addLead: (lead) =>
        set((state) => ({
            ...
            profile: state.profile ? { ...state.profile, leadsReceived: state.profile.leadsReceived + 1 } : null
        })),
    updateLeadStatus: (id, status) =>
        set((state) => ({
            leads: state.leads.map(l => l.id === id ? { ...l, status } : l),
            profile: state.profile && status === 'completed'
                ? { ...state.profile, tripsCompleted: state.profile.tripsCompleted + 1 }
                : state.profile
        })),
    ...
    updateVehicle: (id, updates) =>
        set((state) => ({
            profile: state.profile
                ? {
                    ...state.profile,
                    vehicles: state.profile.vehicles.map(v => v.id === id ? { ...v, ...updates } : v)
                }
                : null
        })),
    removeVehicle: (id) =>
