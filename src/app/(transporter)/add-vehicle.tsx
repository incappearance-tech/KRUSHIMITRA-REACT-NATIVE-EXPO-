...
import { PaymentHistory, useTransporterStore, Vehicle, VehicleDriver } from '@/src/store/transporter.store';
...
const PLANS = [
    ...
    { key: 'quarterly', title: 'Quarterly', price: '₹1400', days: 90, tag: 'Popular', popular: true },
    { key: 'yearly', title: 'Yearly', price: '₹5000', days: 365, tag: 'Best Value' },
];
            ...
            <View style={styles.content}>
                    ...
                    <ScrollView contentContainerStyle={styles.scrollForm} showsVerticalScrollIndicator={false}>
                            ...
                            <View style={styles.stepContainer}>
                                ...
                                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.typeScroll}>
                                    {VEHICLE_TYPES.map((type) => (
                                        <TouchableOpacity
                                            key={type.id}
                                            style={[styles.typeCard, vehicleData.type === type.id && styles.typeCardActive]}
                                            onPress={() => setVehicleData({ ...vehicleData, type: type.id })}
                                        >
                                            <MaterialIcons
                                                name={type.icon as any}
                                                size={24}
                                                color={vehicleData.type === type.id ? COLORS.brand.primary : '#94a3b8'}
                                            />
                                            <Text style={[styles.typeText, vehicleData.type === type.id && styles.typeTextActive]}>
                                                {type.id}
                                            </Text>
                                        </TouchableOpacity>
                                    ))}
                                ...
                                <LabeledInput
                                    ...
                                    onChangeText={(t: string) => setVehicleData({ ...vehicleData, model: t })}
                                />
                                <LabeledInput
                                    label="Vehicle Number (RC)"
                                    placeholder="MH 12 AB 1234"
                                    value={vehicleData.number}
                                    onChangeText={(t: string) => setVehicleData({ ...vehicleData, number: t })}
                                ...
                                <LabeledInput
                                    ...
                                    onChangeText={(t: string) => setVehicleData({ ...vehicleData, capacity: t })}
                        ...
                        {currentStep === 2 && (
                            <View style={styles.stepContainer}>
                                <View style={styles.banner}>
                                    ...
                                    <Text style={styles.bannerText}>Assign a driver to manage trips for this vehicle.</Text>
                                </View>
                                ...
                                <LabeledInput
                                    ...
                                    onChangeText={(t: string) => setDriverData({ ...driverData, name: t })}
                                    icon="person"
                                />
                                <LabeledInput
                                    label="Phone Number"
                                    ...
                                    onChangeText={(t: string) => setDriverData({ ...driverData, phone: t })}
                                ...
                                <LabeledInput
                                    ...
                                    onChangeText={(t: string) => setDriverData({ ...driverData, licenseNo: t })}
                                    icon="badge"
                                />
                            </View>
                        )}
                        ...
                        {currentStep === 3 && (
                            <View style={styles.stepContainer}>
                                ...
                                <Text style={styles.headerSub}>Choose a plan to list your vehicle and get leads.</Text>
                                    ...
                                    {PLANS.map((plan) => (
                                        ...
                                        >
                                            ...
                                            <View style={styles.planHeader}>
                                                ...
                                                <View style={[styles.planRadio, selectedPlanKey === plan.key && styles.planRadioSelected]}>
                                                    {selectedPlanKey === plan.key && <View style={styles.radioInner} />}
                                                </View>
                                            </View>
                                            <Text style={styles.planPrice}>{plan.price}</Text>
                                            <Text style={styles.planDuration}>{plan.days} Days Validity</Text>
                                        </TouchableOpacity>
                                    ))}
                                </View>
                                ...
                                <View style={styles.summaryBox}>
                                    <View style={styles.summaryRow}>
                                        ...
                                        <Text style={styles.summaryVal}>{PLANS.find(p => p.key === selectedPlanKey)?.price}</Text>
                                    </View>
                                    <View style={styles.summaryRow}>
                                    ...
                                    <View style={[styles.summaryRow, styles.totalRow]}>
                                        ...
                                        <Text style={styles.totalVal}>{PLANS.find(p => p.key === selectedPlanKey)?.price}</Text>
                                    </View>
                                </View>
                            </View>
                        )}
                ...
                <View style={styles.footer}>
                    <Button
                        ...
                        onPress={handleNext}
                        backgroundColor={currentStep === 3 ? '#0f172a' : COLORS.brand.primary}
...
const styles = StyleSheet.create({
    ...
    typeCard: { width: 100, height: 100, borderRadius: 16, backgroundColor: '#fff', borderWidth: 1, borderColor: '#e2e8f0', alignItems: 'center', justifyContent: 'center', gap: 8 },
    typeCardActive: { borderColor: COLORS.brand.primary, backgroundColor: '#f0fdf4' },
    typeText: { fontSize: 12, fontWeight: '600', color: '#64748b', textAlign: 'center' },
    typeTextActive: { color: '#166534' },

    banner: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#f1f5f9', padding: 16, borderRadius: 12, marginBottom: 8 },
    ...
    plansGrid: { gap: 16 },
    planCard: { backgroundColor: '#fff', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#e2e8f0', position: 'relative' },
    planCardSelected: { borderColor: COLORS.brand.primary, borderWidth: 2, backgroundColor: '#f0fdf4' },
    popularBadge: { position: 'absolute', top: -10, right: 16, backgroundColor: '#f59e0b', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
    popularText: { fontSize: 10, fontWeight: '800', color: '#fff' },
    planHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
    planTitle: { fontSize: 16, fontWeight: '700', color: '#0f172a' },
    planRadio: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: '#cbd5e1', alignItems: 'center', justifyContent: 'center' },
    ...
    planPrice: { fontSize: 24, fontWeight: '800', color: '#0f172a', marginBottom: 2 },
    planDuration: { fontSize: 12, color: '#64748b' },

    summaryBox: { marginTop: 24, backgroundColor: '#fff', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#e2e8f0' },
