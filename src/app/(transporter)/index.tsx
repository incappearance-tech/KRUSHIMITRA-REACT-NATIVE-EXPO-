...
// Mock Quick Actions
const QUICK_ACTIONS = [
    { title: 'Add Vehicle', icon: 'add-business', color: '#dcfce7', iconCol: '#16a34a', route: '/(transporter)/add-vehicle' }, // Green
    { title: 'My Drivers', icon: 'people', color: '#ffedd5', iconCol: '#ea580c', route: '/(transporter)/drivers' }, // Orange
    { title: 'Trip History', icon: 'history', color: '#f3e8ff', iconCol: '#9333ea', route: '/(transporter)/history' }, // Purple
];
        ...
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                <View style={styles.userBox}>
                    ...
                    <View>
                        <Text style={styles.welcomeLabel}>Fleet Owner</Text>
                        <Text style={styles.userName}>{profile.name}</Text>
                    </View>
                </View>
                <TouchableOpacity onPress={() => router.push('/(transporter)/profile')} style={styles.profileIconBtn}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
                ...
                <View style={styles.metricsContainer}>
                    <View style={styles.metricCard}>
                        <MaterialIcons name="speed" size={24} color={COLORS.brand.primary} />
                    ...
                    <View style={styles.metricCard}>
                        ...
                        <Text style={styles.metricVal}>{profile.vehicles?.length || 0}</Text>
                    ...
                    {QUICK_ACTIONS.map((action, index) => (
                        ...
                        >
                            <View style={[styles.actionIconBox, { backgroundColor: action.color }]}>
                                <MaterialIcons name={action.icon as any} size={24} color={action.iconCol} />
                            </View>
                ...
                {/* 4. TRIP MANAGER */}
                <Text style={styles.sectionTitle}>Trip Manager</Text>
                <View style={styles.tripManagerContainer}>
                    ...
                    <View style={styles.listContainer}>
                        {(activeTab === 'pending' ? pendingLeads : activeLeads).length > 0 ? (
                            (activeTab === 'pending' ? pendingLeads : activeLeads).map((lead) => (
                                <View key={lead.id} style={styles.tripCard}>
                                    <View style={styles.tripHeader}>
                                        <View style={styles.tripBadge}>
                                            <MaterialIcons name="local-shipping" size={14} color="#0f172a" />
                                            <Text style={styles.tripBadgeText}>{lead.vehicleType}</Text>
                                        </View>
                                    ...
                                    <View style={styles.routeRow}>
                                        <View style={styles.routePoint}>
                                            ...
                                            <Text style={styles.routeText} numberOfLines={1}>{lead.pickupLocation}</Text>
                                        </View>
                                        <View style={styles.routeLine} />
                                        <View style={styles.routePoint}>
                                            <View style={styles.dotRed} />
                                            <Text style={styles.routeText} numberOfLines={1}>{lead.dropLocation}</Text>
                                        </View>
                                    </View>

                                    <View style={styles.farmerRow}>
                                        <View style={styles.farmerInfo}>
                                            <MaterialIcons name="account-circle" size={20} color="#94a3b8" />
                            ...
                            <View style={styles.emptyState}>
                                ...
                                <Text style={styles.emptyText}>
                                    {activeTab === 'pending' ? 'No new leads currently.' : 'No active trips.'}
                                </Text>
                            </View>
                        )}
                    </View>
...
const styles = StyleSheet.create({
    ...
    activeIndicator: { position: 'absolute', bottom: 0, right: 0, width: 12, height: 12, borderRadius: 6, backgroundColor: '#22c55e', borderWidth: 2, borderColor: '#fff' },
    welcomeLabel: { fontSize: 11, color: '#64748b', fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5 },
    ...
    metricCard: { flex: 1, backgroundColor: '#fff', borderRadius: 20, padding: 16, alignItems: 'center', borderWidth: 1, borderColor: '#f1f5f9', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8 },
    metricVal: { fontSize: 18, fontWeight: '800', color: '#0f172a', marginVertical: 4 },
    ...
    // Actions
    actionGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, justifyContent: 'space-between', marginBottom: 24 },
    actionCard: {
        width: (width - 44) / 2,
        flexDirection: 'row',
        alignItems: 'center',
    ...
    tabs: { flexDirection: 'row', backgroundColor: '#fff', padding: 4, borderRadius: 12, marginBottom: 16 },
    tab: { flex: 1, paddingVertical: 8, alignItems: 'center', borderRadius: 8 },
    tabActive: { backgroundColor: '#f1f5f9' },
    tabText: { fontSize: 13, fontWeight: '600', color: '#64748b' },
    tabTextActive: { color: '#0f172a' },
    listContainer: { gap: 12 },
    tripCard: { backgroundColor: '#fff', padding: 16, borderRadius: 16, borderWidth: 1, borderColor: '#e2e8f0' },
    tripHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
    tripBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#f1f5f9', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
    ...
    farmerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 12, borderTopWidth: 1, borderTopColor: '#f8fafc' },
    farmerInfo: { flexDirection: 'row', alignItems: 'center', gap: 6 },
    farmerName: { fontSize: 13, fontWeight: '600', color: '#64748b' },

    callSmallBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#16a34a', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
    callSmallText: { color: '#fff', fontSize: 11, fontWeight: '700' },

    actionRow: { flexDirection: 'row', gap: 12, marginTop: 12 },
    rejectBtn: { flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 8, borderWidth: 1, borderColor: '#fee2e2' },
    rejectText: { color: '#ef4444', fontWeight: '700', fontSize: 13 },
    acceptBtn: { flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 8, backgroundColor: COLORS.brand.primary },
