...
export default function TransporterProfile() {
    ...
    const menuItems = [
        ...
        },
        {
            ...
            action: () => Alert.alert('Support', 'Contact Transporter Care: +91 1800-456-789'),
        },
        {
            icon: 'logout',
            title: 'Logout',
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.ratingRow}>
                        ...
                        <Text style={styles.ratingVal}>{profile.rating} • {profile.verified ? 'Verified' : 'Member'}</Text>
                    </View>
                </View>
                ...
                <View style={styles.statsContainer}>
                    <View style={styles.statLine}>
                        <View style={styles.statItem}>
                            ...
                            <Text style={styles.statLabText}>Trips</Text>
                        </View>
                        <View style={styles.vDivider} />
                        <View style={styles.statItem}>
                            <Text style={styles.statValNum}>{profile.operatingRadius}km</Text>
                            <Text style={styles.statLabText}>Radius</Text>
                        </View>
                        <View style={styles.vDivider} />
                        <View style={styles.statItem}>
                            <Text style={styles.statValNum}>{profile.leadsReceived}</Text>
                            <Text style={styles.statLabText}>Leads</Text>
                        </View>
                    </View>
                </View>
                ...
                <View style={styles.section}>
                    <View style={styles.sectionHeader}>
                        <Text style={styles.secTitle}>My Fleet ({profile.vehicles?.length || 0})</Text>
                        <TouchableOpacity onPress={() => router.push('/(transporter)/add-vehicle')}>
                        ...
                        {profile.vehicles && profile.vehicles.length > 0 ? (
                            profile.vehicles.map((v) => (
                                <TouchableOpacity
                                    ...
                                    onPress={() => router.push(`/(transporter)/edit-vehicle?id=${v.id}`)}
                                >
                                    <View style={styles.fleetIcon}>
                                        <MaterialIcons name="local-shipping" size={20} color={COLORS.brand.primary} />
                                    </View>
                                    <View style={styles.fleetInfo}>
                                        <Text style={styles.fleetNumber}>{v.number}</Text>
                                        <Text style={styles.fleetType}>{v.type} • {v.model}</Text>
                                    </View>
                                    <MaterialIcons name="chevron-right" size={20} color="#d1d5db" />
                                </TouchableOpacity>
                            ))
                        ) : (
                            <Text style={styles.emptyFleetText}>No vehicles registered yet.</Text>
                        )}
                    </View>
                </View>
                ...
                <View style={styles.menuBox}>
                    {menuItems.map((item, i) => (
                        <TouchableOpacity key={i} style={styles.mItem} onPress={item.action}>
                            <View style={[styles.mIcon, item.destructive && { backgroundColor: '#fee2e2' }]}>
                                <MaterialIcons name={item.icon as any} size={20} color={item.destructive ? '#dc2626' : COLORS.brand.primary} />
                            </View>
                            <View style={styles.mContent}>
                                <Text style={[styles.mTitle, item.destructive && { color: '#dc2626' }]}>{item.title}</Text>
                                {item.subtitle && <Text style={styles.mSubtitle}>{item.subtitle}</Text>}
                            </View>
                            <MaterialIcons name="chevron-right" size={20} color="#d1d5db" />
                        </TouchableOpacity>
                    ))}
...
const styles = StyleSheet.create({
    ...
    badge: { position: 'absolute', bottom: 0, right: 0, backgroundColor: COLORS.brand.primary, padding: 6, borderRadius: 20, borderWidth: 2, borderColor: '#fff' },
    ...
    statLine: { flexDirection: 'row', backgroundColor: '#fff', padding: 20, borderRadius: 16, borderWidth: 1, borderColor: '#f3f4f6', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 10 },
    statItem: { flex: 1, alignItems: 'center' },
    statValNum: { fontSize: 18, fontWeight: '800', color: COLORS.text },
    statLabText: { fontSize: 11, color: '#9ca3af', marginTop: 4, textTransform: 'uppercase' },
    ...
    vehicleList: { gap: 12 },
    fleetCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 12, borderRadius: 16, borderWidth: 1, borderColor: '#f3f4f6' },
    fleetIcon: { width: 40, height: 40, borderRadius: 10, backgroundColor: COLORS.brand.muted, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
    fleetInfo: { flex: 1 },
    fleetNumber: { fontSize: 14, fontWeight: '700', color: COLORS.text },
    fleetType: { fontSize: 12, color: COLORS.textSecondary, marginTop: 2 },
    emptyFleetText: { fontSize: 14, color: '#9ca3af', fontStyle: 'italic', textAlign: 'center', paddingVertical: 10 },
    menuBox: { paddingHorizontal: 20 },
    mItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
    mIcon: { width: 40, height: 40, borderRadius: 10, backgroundColor: COLORS.brand.muted, alignItems: 'center', justifyContent: 'center', marginRight: 16 },
    mContent: { flex: 1 },
