...
const AvailabilityScreen: React.FC = () => {
      ...
      if (machine && machine.expiry) {
        ...
        if (parts[0].includes('Oct')) {
          const startDay = parseInt(
            parts[0].replace('Oct ', '').replace(', 2023', ''),
          );
          setSelectedStart(startDay || 5);
          if (parts[1]) {
            const endDay = parseInt(
              parts[1].replace('Oct ', '').replace(', 2023', ''),
            );
  ...
  const getDayStatus = (dayNum: number) => {
    ...
    if (dayNum === selectedStart)
      return selectedEnd ? 'selected-start' : 'selected-single';
    if (dayNum === selectedEnd) return 'selected-end';
    if (selectedEnd && dayNum > selectedStart && dayNum < selectedEnd)
      return 'selected-mid';
    return 'none';
  };
  ...
  const onNext = () => {
    ...
    router.push({
      pathname: '/(farmer)/rent-out/publish',
    ...
    <View style={styles.safeArea}>
      ...
      >
          ...
          <View style={styles.machineInfo}>
            ...
            <View>
              <Text style={styles.machineTitle}>
                {draftRental?.name || 'New Machine'}
              </Text>
              <Text style={styles.machineSub}>
                {draftRental?.type || 'Category'}
              </Text>
            </View>
          </View>
        ...
        <Text style={styles.sectionSub}>
          Tap dates to mark when your machine is free.
        </Text>
          ...
          <View style={styles.buttonGroup}>
            {(['Morning', 'Afternoon', 'Evening'] as const).map((time) => (
              <TouchableOpacity
                ...
                style={[
                  styles.prefButton,
                  pickupTime === time && styles.activePrefButton,
                ]}
              >
                <Text
                  style={[
                    styles.prefButtonText,
                    pickupTime === time && styles.activePrefText,
                  ]}
                >
                  {time}
                </Text>
